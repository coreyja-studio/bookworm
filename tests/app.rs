...
async fn library_search_filters_results() {
    ...
    let book_id_a: uuid::Uuid = sqlx::query_scalar(
        "INSERT INTO books (title, author) VALUES ('Alpha Search Book', 'Author A') RETURNING book_id"
    )
    .fetch_one(&db)
    .await
    .unwrap();
    ...
    let book_id_b: uuid::Uuid = sqlx::query_scalar(
        "INSERT INTO books (title, author) VALUES ('Beta Other Book', 'Author B') RETURNING book_id"
    )
    .fetch_one(&db)
    .await
    .unwrap();
